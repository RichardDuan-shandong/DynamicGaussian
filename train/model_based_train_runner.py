...
class ModelBasedTrainRunner(TrainRunner):
    ...
    def __init__(self,
        ...
        super(ModelBasedTrainRunner, self).__init__(
    ...
    def start(self):
        ...
        for i in range(start_iter, self._iterations):
            # 标记当前是第几步，即更新self._step_signal.value
            self._env_runner.set_step(i)
            # 有训练环境或者没有评估环境
            if self._num_train_envs > 0 or self._num_eval_envs == 0:
                log_iteration = (i % self._log_freq == 0) and (i > 0) # 每self._log_freq步生成一次log记录
            else:
                # 获取当前评估环境中的评估回合数
            ...
            # 每一轮都需要收集到足够的replay buffer数据才会继续进行更新，也就是需要进行一段时间的采样
