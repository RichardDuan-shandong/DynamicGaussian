...
class SkeletonSplatter(nn.Module):
    def __init__(self, cfg, device_ids):
        ...
        self.depth_predictor = SkeletonRecon(cfg, device_ids)
...
class GaussianRenderer(nn.Module):
    def __init__(self, cfg, device_ids):
        ...
        self.voxel_encoder = torch.nn.DataParallel(MultiLayer3DEncoderShallow(in_channels=7, out_channels=cfg.gaussian_renderer.final_dim), device_ids=device_ids)
        
        self.gaussian_regressor = GaussianRegressor(cfg, device_ids)
    ...
    def forward(self, data, pcds, pcds_project_to_image, feature_map):
        ...
        for _pcds_flat in pcds_flat:
            single_batch_voxel_grids, single_batch_voxel_density = self._voxelizer.coords_to_bounding_voxel_grid(
                _pcds_flat, coord_features=None, coord_bounds=torch.tensor(self.cfg.rlbench.scene_bounds).to(device=device), return_density=True)
        ...
        return predict_render_images, new_pcds, gaussian_params_list