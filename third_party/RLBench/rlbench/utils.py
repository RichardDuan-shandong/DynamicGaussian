...
def get_stored_demos(amount: int, image_paths: bool, dataset_root: str,
    ...
    if variation_number == -1:  # default:variation_number=1
        # All variations
        examples_path = join(
    ...
    if amount == -1:    # default:amount=1 如果是-1代表把disk中所有存储的数据都拿来放入到缓冲区
        amount = len(examples)
    if amount > len(examples):
        raise RuntimeError(
            'You asked for %d examples, but only %d were available.' % (
                amount, len(examples)))
    if random_selection:
        selected_examples = np.random.choice(examples, amount, replace=False)   # 随机从varition的任务中挑选amount个
    else:
