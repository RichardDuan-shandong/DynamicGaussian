...
class TaskUniformReplayBuffer(UniformReplayBuffer):
    ...
    # 在多线程环境下，安全地将当前游标（cursor）添加到某个任务的索引列表（self._task_idxs[task]）中
    def _add(self, kwargs: dict):
        ...
        with self._lock:
            ...
            if self._disk_saving:   # default: True
                ...
                for k, v in kwargs.items(): # 遍历key, value值
                    try:
                        if 'float' in v.dtype.name  and v.size > 100:
                            v = v.astype(np.float16)    # 使用float16的np存储value值
                            kwargs[k] = v               # 任务名称
                    except:
                        pass
                # 在本地磁盘保存采样的结果
    ...
    # 返回replay buffer的name，类型及维度大小信息
    def get_transition_elements(self, batch_size=None):
        ...
        """
        # 如果batch_size为空则使用默认的batch_size=32
