...
class UniformReplayBuffer(ReplayBuffer):
    ...
    # 创建一个保存（replay buffer的存储核心结构就是这个self._store，他是由_create_storage来定义格式的）
    def _create_storage(self, store=None):
        """Creates the numpy arrays used to store transitions.
        """
        self._store = {} if store is None else store    # 传入一个带内容的store，这个store可能是存储有各种各样的东西，如果不是空就直接给store
    ...
    # 这是一个工具函数，用于获取到当前传入的observation和extra的elem的格式
    ...
    # 添加回合进行中每一个时间步的转移，将action reward termial timeout三个元素加入到replay buffer中
    ...
    # 添加回合结束时的最终转移，transition[TERMINAL]被赋值为-1
    ...
    def _add_initial_to_disk(self ,kwargs: dict):
        for i in range(self._timesteps - 1):
            with open(join(self._save_dir, '%d.replay' % (
                ...
                pickle.dump(kwargs, f)  # 将 Python 对象（在这里是 kwargs）序列化并存储到一个文件对象 f 中


    '''
        self._store = {
        'action': np.array([[1], [2], [3], ..., [n]]),  # 存储每个时间步的动作
        'reward': np.array([0.5, 1.0, -0.5, ..., reward_n]),  # 存储每个时间步的奖励
        'terminal': np.array([0, 0, 1, ..., terminal_n]),  # 存储每个时间步的终止标志
        'timeout': np.array([False, True, False, ..., timeout_n]),  # 存储每个时间步的超时标志
        # 其他可能的转移数据
        }
    '''

    # （写，最底层的，包括直接写到磁盘厘米）向内存空间中保存replay buffer内容
    ...
    # 检查将要放入replay buffer的kwargs结构是否符合signature（即要求的replay buffer中的element格式）
    ...
    # 游标，其实就是一个指针，指向下一个将要插入的位置（由于是wrapped_replay_buffer，因此是一个循环队列指针）
    def cursor(self):
    ...
    # （读）从replay buffer采样一个batch_size大小的transition
    def sample_transition_batch(self, batch_size=None, indices=None,
        ...
        with self._lock:    # 使用线程锁
            ...
            assert len(indices) == batch_size   # 确保采样到的indices的长度和batch_size一致

            transition_elements = self.get_transition_elements(batch_size)  # 获取transition的元素的格式
            batch_arrays = self._create_batch_arrays(batch_size)    # 预分配一个空的数组，存放采样的批量数据
            ...
            for batch_element, state_index in enumerate(indices):
                ...
                # trajectory_indices 是一个包含从当前状态索引 state_index 开始的连续若干个状态的索引列表，
                # 长度为 self._update_horizon。这是为了构建一个状态到终止状态之间的轨迹
    ...
    def get_transition_elements(self, batch_size=None):
        ...
        # action reward terminal timeout indieces
        transition_elements = [
