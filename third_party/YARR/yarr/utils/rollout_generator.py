...
class RolloutGenerator(object):
    ...
    def generator(self, step_signal: Value, env: Env, agent: Agent,
        ...
        agent.reset()
        # 这里用于动作决策的输入观测状态的obs_history长度为timesteps，也就是一次动作决策前要输入前timesteps长度的动作
        obs_history = {k: [np.array(v, dtype=self._get_type(v))] * timesteps for k, v in obs.items()}
        lang_goal = env._task.get_task_descriptions()[0]

        # obs先获得最初始状态的obs，然后进行episode_length步的探索
        for step in range(episode_length):
            ...
            # 与环境进行交互，进行一次状态转移(extract_obs包含在其中)
            ...
            # 维护一个固定长度序列的obs_history:每次探索完一步就加入到obs中，再丢出去一个最老的
