...
class OfflineTrainRunner():
    ...
    # data_iter 是一个迭代器，每次调用 next(data_iter) 会返回一个新的数据批次
    ...
    def start(self):
        ...
        # 开始迭代
        for i in tqdm(range(start_iter, self._iterations), mininterval=self.tqdm_mininterval):  # 显示进度条，每隔self.tqdm_mininterval就更新一次进度条
