...
''''
 这是一个格式自适应的replay buffer填充器
 通过预先定义好将要读入replay buffer的格式（在create_replay中定义）
 首先是定义replay buffer的格式(create_replay)
 然后将磁盘中保存
'''
import logging
...
from agents.dyna_q_agent.manigaussian_dyna_q.perceiver_lang_io import PerceiverVoxelLangEncoder
from agents.dyna_q_agent.manigaussian_dyna_q.dyna_q_manigaussian_agent import dyna_q_manigaussian_agent
...
def create_replay(batch_size: int, timesteps: int,
    ...
    if not single_process:  # default: False
        replay_buffer = TaskUniformReplayBuffer(
            ...
            action_shape=(8,),  # 动作格式是一个一维数组
            action_dtype=np.float32,
            reward_shape=(),    # reward是一个标量格式
...
def fill_replay(cfg: DictConfig,
    ...
    for d_idx in range(num_demos):
        # load demo from disk
        demo = rlbench_utils.get_stored_demos(      # 获取专家的演示(只从每个任务中随机读入一个专家演示)
...
def fill_multi_task_replay(cfg: DictConfig,
                           ...
                           keypoint_method = 'heuristic',
                           fabric: Fabric = None):
    manager = Manager() # Manager 是 multiprocessing 模块提供的一个类，它能够帮助你创建可以在多个进程之间共享的数据结构。通常，使用 Manager 创建的数据结构（比如字典、列表等）是进程安全的，可以在多个进程之间同步
    store = manager.dict() #  创建了一个可以在多个进程间共享的字典 store

    # create a MP dict for storing indicies
    # TODO(mohit): this shouldn't be initialized here
    # 这两行代码检查 replay 对象是否已经有 _task_idxs 属性。如果已经存在，则删除该属性。这通常是为了防止在不同的进程间重复初始化或者存储旧的值
