...
import torch
from train.bc import train_env_launch_bc
from helpers.utils import create_obs_config
import lightning as L


@hydra.main(config_name='offlinerl_config', config_path='conf')
    ...
    for seed in range(start_seed, start_seed + cfg.framework.seeds):
        ...
        if cfg.method.use_fabric:
            ...
            try:
                ...
                pass
            mp.spawn(train_env_launch_bc.run_seed,
                     args=(cfg,
                           obs_config,
                           cfg.rlbench.cameras,
                           multi_task,
