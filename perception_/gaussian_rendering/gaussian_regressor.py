...
from perception_.gaussian_rendering.resnetfc import ResnetFC
from perception_.gaussian_rendering.utils  import PositionalEncoding, visualize_pcd
...
class GaussianEncoder(nn.Module):
    def __init__(self, cfg, fabric, out_channels):
        super().__init__()
        self.cfg = cfg
        self.fabric = fabric
        ...
        self.code = PositionalEncoding.from_conf(cfg["code"], d_in=d_in).to(device=fabric.device)
        ...
        if self.fabric is not None:
            self.encoder = self.fabric.setup_module(ResnetFC(
                    d_in=d_in, # xyz
...
class GaussianRegressor(nn.Module):
    def __init__(self, cfg, fabric):
        ...
        if self.fabric is not None:
            self.gs_encoder = self.fabric.setup_module(GaussianEncoder(
                cfg,
                fabric,
                split_dimensions,
            ))
            
            self.gs_decoder = self.fabric.setup_module(GuassianDecoder(
                cfg,
                split_dimensions,
                scale=scale_inits,
                bias=bias_inits,
    ...
    def forward(self, pcds, pcds_project_to_image, voxel_feature, image_feature):
        ...
        xyz_maps, opacity_maps, scale_maps, rot_maps, features_dc_maps = split_network_outputs[:5]
        ...
        optimizer_gaussian_decoder = torch.optim.Adam(self.gs_encoder.parameters(), lr=1e-3)
        ...
        return gaussian_params, test_loss