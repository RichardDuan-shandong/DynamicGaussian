...
def render(calib, idx, pts_xyz, rotations, scales, opacity, bg_color, pts_rgb=None, features_color=None, features_language=None):
    """
    Render the scene. 
    ...
    tanfovx = math.tan(calib['FovX'][idx] * 0.5)
    tanfovy = math.tan(calib['FovY'][idx] * 0.5)

    raster_settings = GaussianRasterizationSettings(
        image_height=int(calib['height'][idx]),
        image_width=int(calib['width'][idx]),
        tanfovx=tanfovx,
        tanfovy=tanfovy,
        bg=bg_color,
        scale_modifier=1.0,
        viewmatrix=calib['world_view_transform'][idx],
        projmatrix=calib['full_proj_transform'][idx],
        sh_degree=3 if features_color is None else 1,
        campos=calib['camera_center'][idx],
        prefiltered=False,
        debug=False,
    ...
    rendered_image, language_feature_image, radii = rasterizer(
        ...
        language_feature_precomp=None, # 暂时禁用lang_rendering
        opacities=opacity,
        scales=scales,
        rotations=rotations,
        cov3D_precomp=None,
