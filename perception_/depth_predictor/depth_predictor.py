...
from perception_.depth_predictor.unet_part_utils import Linear, Conv2d, PositionalEmbedding
from perception_.depth_predictor.SongUnet import SongUNet
from perception_.utils import fov2focal, focal2fov, world_to_canonical, canonical_to_world, getView2World
from sklearn.mixture import GaussianMixture
from perception_.depth_predictor.keypoint_sample_utils import get_keypoint
...
class SkeletonRecon(nn.Module):
    ...
    def reshape_out(self, means, depth, offset, sigma_scale, pi_scale):
        x_indices = means[:, :, 0].to(device=self.fabric.device).long()  # (B, N)
        y_indices = means[:, :, 1].to(device=self.fabric.device).long()  # (B, N)
    ...
    def forward(self, x):
        ...
        filtered_rays = self.filter_ray(means) 
        # 3.depth predict
        ...
        depth, offset, sigma_scale, pi_scale = depth.to(device=self.fabric.device), offset.to(device=self.fabric.device), sigma_scale.to(device=self.fabric.device), pi_scale.to(device=self.fabric.device)
        means, cov, pi, object_index = means.to(device=self.fabric.device), cov.to(device=self.fabric.device), pi.to(device=self.fabric.device), object_index.to(device=self.fabric.device)
        filtered_rays = filtered_rays.to(device=self.fabric.device)
        for i in range(B):
