...
class SkeletonGMM():
    # input: 2d
    def __init__(self, means:torch.Tensor, covariances:torch.Tensor, weights:torch.Tensor, object_index:torch.Tensor, fabric : Fabric, extr, intr, cfg) ->None :
        self.means = means
    ...
    def cov_expand_to_3D(self, sigma_scale):
        ...
        sigma_x = self.covariances[:, 0, 0] * sigma_scale[:, 0]
        sigma_y = self.covariances[:, 1, 1] * sigma_scale[:, 1]

        # 计算非对角线项
        scale_xy = torch.sqrt(sigma_scale[:, 0] * sigma_scale[:, 1])
        cov_xy = self.covariances[:, 0, 1] * scale_xy  # 由于 cov 是对称的，cov[0,1] == cov[1,0]
